#![recursion_limit="512"] // Weird hack I had to stack-overflow for.  The html! macro doesn't compile otherwise
...
impl Model {
  fn view_task(&self, task: &String, index: u32) -> Html {
    html! {
      <li class="li">
        {task}
        <button class="button" onclick=self.link.callback(|_| Msg::RemoveTask(index))>
          {"Delete"}
        </button>
      </li>
    }
  ...
  fn update(&mut self, msg: Self::Message) -> ShouldRender {
    match msg {
      ...
      Msg::RemoveTask(index) => {
        self.state.tasks.remove(index as usize);
      }
      Msg::RemoveAllTasks => {
  ...
  fn view(&self) -> Html {
    html! {
      <div class="background">
        <form class="form" onsubmit={self.link.callback(|e: SubmitEvent| Msg::AddTask(e))}>
        ...
        <ul class="ul">
          {for self.state.tasks.iter().enumerate().map(|task, i| self.view_task(task, i))}
        </ul>
